...
const musicList = [
    "comical-orchestra-comedy-music.wav",
    "little-and-curious-fun-quirky.wav",
    "quirky-game-music-gaming-land.wav",
    "quirky-jaunty-whimsical-advent.wav",
    "sneaking-around-funny-comedy-t.wav"
]
...
const toggleMute = () => {
    if (isMute) {
        ...
        gameSounds.splash.setVolume(0.5);
        isMute = false;
...
//creates fox
class Fox {
    constructor(size = { x: 0.1, y: 0.1, z: 0.1 }) {
        ...
        const materialLoader = new THREE.MTLLoader(loadManager);
        materialLoader.load("assets/model/redfox.mtl", materials => {
            materials.preload();
            const objLoader = new THREE.OBJLoader(loadManager);
            objLoader.setMaterials(materials).load("/assets/model/redfox.obj", model => {
                this.model.add(model);
                this.model.scale.set(size.x, size.y, size.z);

                let maxHeight = 1.2, minHeight = 1, duration = 1.2;
    ...
    getLane() {
        return -Math.round(fox.model.position.z / cellWidth);
    }
...
class Sound {
    constructor(listenerParent) {
        ...
        // audioLoader.load('assets/audio/death2.mp3', buffer => {
...
const update = () => {
    ...
    if (fox) {
        //animations
        if (fox.jumpAnimation && fox.jumpAnimation.mixer)
            fox.jumpAnimation.mixer.update(fox.jumpAnimation.clock.getDelta());
        if (fox.sizeAnimation && fox.sizeAnimation.mixer && !gameOver)
            fox.sizeAnimation.mixer.update(fox.sizeAnimation.clock.getDelta());
        if (fox.heightAnimation && fox.heightAnimation.mixer)
            fox.heightAnimation.mixer.update(fox.heightAnimation.clock.getDelta());
        if (fox.fallAnimation && fox.fallAnimation.mixer)
            fox.fallAnimation.mixer.update(fox.fallAnimation.clock.getDelta());
        fox.feathers.animations.forEach((animation, index) => {
            if (animation && animation.mixer) {
                ...
                if (animation.mixer.time > animation.firstInterval)
                    fox.feathers.feathers[index].rotation.x += animation.rotationSpeed * deltaTime;
            }
        });
        fox.splashes.animations.forEach(animation => {
            if (animation && animation.mixer)
                animation.mixer.update(animation.clock.getDelta());
        });

        //makes camera follow player
        camera.position.x = fox.model.position.x + cameraOffsetX;
        camera.position.z = fox.model.position.z + cameraOffsetZ;
    }

    if (lanes) {
        //moves vehicles and logs in field of view and checks collision with fox
        lanes.filter(lane => lane.index >= fox.getLane() - 9 && lane.index <= fox.getLane() + 9).forEach(lane => {
            if (lane.type == 'car') {
                ...
                lane.vehicles.forEach(car => {
                    ...
                    const foxLeftEdge = fox.model.position.x - cellWidth / 2 * 0.2;
                    const foxRightEdge = fox.model.position.x + cellWidth / 2 * 0.2;
                ...
                lane.logs.forEach(log => {
                    ...
                    if (foxRightEdge > logLeftEdge && foxLeftEdge < logRightEdge && fox.getLane() == lane.index && fox.isMoving == false && !gameOver) {
                        logsBelowfox++;
                        fox.currentColumn = Math.floor((fox.model.position.x + columns / 2 * cellWidth) / cellWidth);
                        if (fox.currentColumn >= columns)
                            fox.currentColumn = columns - 1;
                        if (fox.currentColumn < 0)
                            fox.currentColumn = 0;
                        if (foxRightEdge < farRight && foxLeftEdge > farLeft)
                            fox.model.position.x += lane.direction ? lane.speed * deltaTime : -lane.speed * deltaTime;
                    }
                });
                if (logsBelowfox == 0 && fox.getLane() == lane.index && fox.isMoving == false) {
                    if (!gameOver) {
                        fox.fall();
                        gameOver = true;
                        setTimeout(() => {
                ...
                const foxLeftEdge = fox.model.position.x - cellWidth / 2 * 0.2;
                const foxRightEdge = fox.model.position.x + cellWidth / 2 * 0.2;
                if (foxRightEdge > trainLeftEdge && foxLeftEdge < trainRightEdge && fox.getLane() == lane.index) {
                    if (!gameOver) {
                        fox.shred();
                        fox.model.visible = false;
                        gameSounds.themeSong.setVolume(0);
...
const onKeyDown = event => {
    switch (event.keyCode) {
        ...
        case 40:
            if (fox)
                fox.jump("down");
            //splashes.animate(fox.model.position);
            break;
        case 32:
            document.body.removeChild(document.getElementById('splash'));
            break;
